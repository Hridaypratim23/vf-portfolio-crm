...
import { updateClient, saveSubHistory } from '../../../lib/db';
...
export const POST: APIRoute = async ({ params, request, locals }) => {
  ...
  const data = {
    ...
    manual_contract_value:    parseFloat(String(form.get('manual_contract_value') || '0')) || 0,
    manual_contract_currency: String(form.get('manual_contract_currency') || 'USD').trim(),
  };
  ...
  await Promise.all([
    ...
    saveSubHistory(db, id, subEntries),
  ]);
